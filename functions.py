...
def arduino_establish_serial_connection(com_port):
    try:
        # Set up the serial connection
        ser = serial.Serial(
            port=com_port,           # Serial port
...
def run_test(PCL_serial_port,t10a_serial_port,arduino_serial_port,warm_up_time,num_angles,output_csv_location):
    ...
    arduino_serial=arduino_establish_serial_connection(arduino_serial_port)
    ...
    for light in lights:
        ...
        for angle in angles:
            ...
            f=open(output_csv_location,"a")
            f.write(str(angle)+','+str(encoder_value)+','+str(lx_value_rounded)+','+str(dimming_voltage_value)+'\n')
            f.close()
